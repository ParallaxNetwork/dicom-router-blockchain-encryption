...
from utils import config, oauth2
import requests
import os
...
def get_service_request(accessionNumber):
    ...
    path = config.fhir_pathsuffix + "/ServiceRequest?identifier=http://sys-ids.kemkes.go.id/acsn/" + \
        config.organization_id + "%7C" + accessionNumber + '&_sort=-_lastUpdated&_count=1'
    res = requests.get(url=config.url+path, headers=headers)
    data = res.json()
...
def get_imaging_study(accessionNumber, token):
    ...
    path = config.fhir_pathsuffix + "/ImagingStudy?identifier=http://sys-ids.kemkes.go.id/acsn/" + \
        config.organization_id + "%7C" + accessionNumber + '&_sort=-_lastUpdated&_count=1'
    res = requests.get(url=config.url+path, headers=headers)
    data = res.json()
    if (data["resourceType"] == "Bundle" and data["total"] >= 1):
...
def imagingstudy_post(filename, id):
    ...
    if id == None:
        res = requests.post(url=config.url+config.fhir_pathsuffix +
                            "/ImagingStudy", data=payload, headers=headers)
    else:
        res = requests.put(url=config.url+config.fhir_pathsuffix +
                           "/ImagingStudy/"+id, data=payload, headers=headers)
    data = res.json()
    LOGGER.info(data)
    LOGGER.info(payload)
...
def dicom_push(assocId, study_iuid, imagingStudyID):
    ...
    for n in range(len(instances)):
        ...
        filename = os.getcwd()+config.dcm_dir+subdir+"/"+study_iuid+"/" + \
            series_iuid+"/"+instance_uid+".dcm"
        try:
            payload = open(filename, 'rb')
            str = ""
            res = requests.post(url=config.url+config.dicom_pathsuffix,
                                data=payload, headers=headers)
            str = res.text
            LOGGER.info("Sending Instance UID: " +
                  series_iuid+"/"+instance_uid+" success")
            dbq.Update(dbq.UPDATE_INSTANCE_STATUS_SENT, [
        ...
        if (str.find("Instance already exists") >= 0):
            ...
            # Remove Instance UID
            os.remove(os.getcwd()+config.dcm_dir+subdir+"/"+study_iuid +
                      "/"+series_iuid+"/"+instance_uid+".dcm")

            # Remove Series UID Folder if Empty
            os.rmdir(os.getcwd()+config.dcm_dir+subdir+"/"+study_iuid+"/"+series_iuid)
